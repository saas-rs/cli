...
use crate::protocol::saas_rs::user::v1::FindIssueCommentRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;
...
pub async fn run(id: String, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = FindIssueCommentRequest { id };
    let res = client.find_issue_comment(req).await?.into_inner();

    // Convert to dataframe in preparation for output
    let json = serde_json::to_vec(&res.issue_comment)?;
