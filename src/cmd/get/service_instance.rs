use crate::apiclient;
use crate::cmd::list::service_instances;
use crate::protocol::saas_rs::user::v1::FindServiceInstanceRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;
...
pub async fn run(id: String, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = FindServiceInstanceRequest { id };
    let res = client.find_service_instance(req).await?.into_inner();

    // Convert to dataframe in preparation for output
    let json = serde_json::to_vec(&res.service_instance)?;
    ...
    let mut df = match output.as_str() {
        "ps" => {
            let cols: Vec<_> = service_instances::PS_COLUMNS
                .iter()
