use crate::apiclient;
use crate::protocol::saas_rs::user::v1::FindManyIssuesRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;

pub const PS_COLUMNS: &[&str] = &["id", "type", "state", "title"];
...
pub async fn run(offset: Option<u32>, limit: Option<u32>, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = FindManyIssuesRequest {
        filter: None,
        field_mask: None,
        limit,
        offset,
    };
    let res = client.find_many_issues(req).await?.into_inner();
    ...
    let json = serde_json::to_vec(&res.issues)?;
