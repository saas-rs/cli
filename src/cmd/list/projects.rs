use crate::apiclient;
use crate::protocol::saas_rs::user::v1::FindManyProjectsRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;
...
pub async fn run(offset: Option<u32>, limit: Option<u32>, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = client.find_many_projects(req).await?.into_inner();
    ...
    let json = serde_json::to_vec(&res.projects)?;
