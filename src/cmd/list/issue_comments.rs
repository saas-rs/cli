use crate::apiclient;
use crate::protocol::saas_rs::user::v1::FindManyIssueCommentsRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;

pub const PS_COLUMNS: &[&str] = &["id", "issueId", "createdAt", "description"];
...
pub async fn run(offset: Option<u32>, limit: Option<u32>, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = FindManyIssueCommentsRequest {
        filter: None,
        field_mask: None,
        limit,
        offset,
    };
    let res = client.find_many_issue_comments(req).await?.into_inner();

    // Without a schema, Polars chokes parsing an empty json array
    if res.issue_comments.is_empty() {
        return super::output_empty_result_of_unknown_schema(&output);
    }

    // Convert to dataframe in preparation for output
    let json = serde_json::to_vec(&res.issue_comments)?;
