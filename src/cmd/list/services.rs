use crate::apiclient;
use crate::protocol::saas_rs::user::v1::FindManyServicesRequest;
use clap::Parser;
use polars::prelude::*;
use std::io::Cursor;
...
pub async fn run(offset: Option<u32>, limit: Option<u32>, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = FindManyServicesRequest {
        filter: None,
        field_mask: None,
        limit,
        offset,
    };
    let res = client.find_many_services(req).await?.into_inner();
    ...
    let json = serde_json::to_vec(&res.services)?;
